...
Class Master extends DBConnection {
	...
	function place_order(){
		...
		if($resp['status'] == 'success'){
			...
			$mailer->SendMail($_SESSION['userdata']['email'], $subject, $message);
	...
	function update_order_status(){
		...
		$user_res = $this->conn->query("SELECT customer_list.*, order_list.code, order_list.total_amount, order_list.delivery_address, order_list.status, order_list.date_created FROM order_list JOIN customer_list ON order_list.customer_id = customer_list.id WHERE order_list.id = '{$id}' ");
        while($row = $user_res->fetch_assoc()){
			...
			$orderStatuses = ['pending', 'packed', 'out for delivery','paid'];
			$orderStatus = $orderStatuses[$row['status']];
			$message = "<div>
